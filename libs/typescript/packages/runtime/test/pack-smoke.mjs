...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { CIP4 as FocusedCIP4 } from "@xray-network/xray-cardano-lib-cip/cip4";',
      'import { CIP8Message as FocusedCIP8Message, COSESign1Builder, HeaderMap, Headers, ProtectedHeaderMap } from "@xray-network/xray-cardano-lib-cip/cip8";',
      'import { encode_asset_name_label } from "@xray-network/xray-cardano-lib-cip/cip67";',
      'import { ProvisionalGovernanceActionId } from "@xray-network/xray-cardano-lib-cip/cip129";',
      ...
      "if (cardano.core.CardanoError !== core.CardanoError ||",
      "    cardano.chain.MultiEraBlock !== multiEra.MultiEraBlock ||",
      "    cardano.chain.BabbageTransactionBody !== BabbageTransactionBody ||",
      "    cardano.chain.discover_required_witnesses !== chain.discover_required_witnesses ||",
      "    cardano.plutus.evaluatePhaseTwo !== plutus.evaluatePhaseTwo ||",
      "    cardano.plutus.SerializedPlutusScript !== plutus.SerializedPlutusScript ||",
      "    cardano.plutus.Data !== Data || cardano.plutus.Constr !== plutus.Constr ||",
      "    cardano.cips.cip4.CIP4 !== FocusedCIP4 ||",
      "    cardano.cips.cip8.CIP8Message !== FocusedCIP8Message ||",
      "    cardano.cips.cip67.encode_asset_name_label !== encode_asset_name_label) {",
      '  throw new Error("packages did not preserve nominal class identity");',
      "}",
      'if ("apply_params_to_script" in plutus || "eval_phase_two_raw" in plutus) {',
      '  throw new Error("deprecated phase-two aliases remain exported");',
      "}",
      'if ("cip129" in cardano.cips || "cip129" in cip) {',
      '  throw new Error("provisional CIP-129 binding leaked into a stable root");',
      "}",
      'if (!ProvisionalGovernanceActionId.from_parts(cardano.crypto.TransactionHash.from_raw_bytes(new Uint8Array(32)), 17).to_bech32().startsWith("gov_action1")) {',
      '  throw new Error("packed CIP-129 focused entry point failed");',
      "}",
      ...
      "}",
      'const cip4 = cardano.cips.cip4.CIP4.calculateChecksum("13559f50b21a7229275ea7586e5b5a504c8ed1d9ddf17464c413f0ba");',
      'if (cip4.checksumId !== "XPPX-4012" || cip4.checksumImage.length !== 128) {',
      '  throw new Error("packed CIP-4 checksum failed");',
      "}",
      ...
      "}",
      'const cip8Address = `60${"11".repeat(28)}`;',
      'const cip8Payload = "deadbeef";',
      'const cip8Envelope = cardano.cips.cip8.CIP8Message.signData(cip8Address, cip8Payload, key.to_bech32());',
      'if (!cardano.cips.cip8.CIP8Message.verifyData(cip8Address, key.to_public().hash().to_hex(), cip8Payload, cip8Envelope)) {',
      '  throw new Error("packed CIP8Message facade failed");',
      "}",
      'if (chain.PlutusData.from_json(\'{"int":42}\').as_integer()?.to_str() !== "42") {',
      '  throw new Error("packed chain Plutus data parser failed");',
      "}",
      'const packedCostModels = chain.CostModels.from_json(\'{"0":[1,-2,3],"1":[4],"2":[5]}\');',
      'if (cardano.chain.CostModels !== chain.CostModels || JSON.stringify(JSON.parse(packedCostModels.to_json())) !== \'{"0":[1,-2,3],"1":[4],"2":[5]}\') {',
      '  throw new Error("packed CostModels identity or JSON consumption failed");',
      "}",
      ...
      "}",
      'if (cardano.plutus.Data.to(undefined, cardano.plutus.Data.Void()) !== "d87980" ||',
      '    cardano.plutus.Data.from("d87980", cardano.plutus.Data.Void()) !== undefined ||',
      '    !(new cardano.plutus.Constr(0, []) instanceof plutus.Constr)) {',
      '  throw new Error("packed aggregate typed Data exports failed");',
      "}",
      'const uplcProgram = uplc.parseUplcText("(program 1.0.0 (con unit ()))");',
      'const serializedUplc = plutus.SerializedPlutusScript.from_raw_flat(uplc.encodeFlatProgram(uplcProgram));',
      'if (serializedUplc.to_double_cbor().length <= serializedUplc.to_single_cbor().length) {',
      '  throw new Error("packed explicit serialized-script envelope failed");',
      "}",
      'const ledgerScript = chain.Script.new_plutus_v1(chain.PlutusV1Script.new(serializedUplc.to_single_cbor()));',
      'if (chain.ScriptRef.new(ledgerScript).script().kind() !== chain.ScriptKind.PlutusV1 || chain.RequiredSigners.new().to_canonical_cbor_hex() !== "d9010280") {',
      '  throw new Error("packed typed Script, ScriptRef, or RequiredSigners API failed");',
      "}",
      ...
      "}",
      "if (cip.cip4.CIP4 !== FocusedCIP4 || cip.cip25.CIP25Metadata === undefined || cip.cip8.COSESign1Builder !== COSESign1Builder || cip.cip67.encode_asset_name_label !== encode_asset_name_label || plutus.Data !== Data || plutus.data.Data !== Data || plutus.uplc.parseUplcText !== uplc.parseUplcText) {",
  ...
  await writeFile(
    ...
    [
      ...
      'import type { CIP4Checksum } from "@xray-network/xray-cardano-lib-cip/cip4";',
      'import type { CIP8MessageEnvelope } from "@xray-network/xray-cardano-lib-cip/cip8";',
      'import * as cip129 from "@xray-network/xray-cardano-lib-cip/cip129";',
      'import * as plutus from "@xray-network/xray-cardano-lib-plutus";',
      'import { Data } from "@xray-network/xray-cardano-lib-plutus/data";',
      'import * as uplc from "@xray-network/xray-cardano-lib-plutus/uplc";',
      'import * as core from "@xray-network/xray-cardano-lib-core";',
      'import * as crypto from "@xray-network/xray-cardano-lib-crypto";',
      'import type { PlutusDataJSON, TransactionMetadatumJSON } from "@xray-network/xray-cardano-lib-chain";',
      'import type { DataSchema, VoidSchema } from "@xray-network/xray-cardano-lib-plutus";',
      ...
      "const datumSchema = Data.Object({ owner: Data.Bytes(), amount: Data.Integer() });",
      "const voidSchema: VoidSchema = cardano.plutus.Data.Void();",
      "const anySchema: DataSchema = voidSchema;",
      'const cip8Envelope: CIP8MessageEnvelope = { signature: "", key: "" };',
      'const cip4Checksum: CIP4Checksum = { checksumId: "XPPX-4012", checksumImage: "" };',
      "type DatumValue = Data.Static<typeof datumSchema>;",
      "type VoidValue = cardano.plutus.Data.Static<typeof voidSchema>;",
      ...
      "const voidValue: VoidValue = undefined;",
      'const packedCostModels: chain.CostModels = cardano.chain.CostModels.from_json(\'{"0":[1,-2,3]}\');',
      "const eraBlock = null as unknown as EraBlockJSON;",
      "Data.to(datumValue, datumSchema);",
      "cardano.plutus.Data.to(voidValue, voidSchema);",
      "void new cardano.plutus.Constr(0, []);",
      "void cardano.plutus.applyParamsToScript;",
      "void cardano.plutus.evaluatePhaseTwoRaw;",
      "void cardano.plutus.evaluatePhaseTwo;",
      "void chain.discover_required_witnesses;",
      "void plutus.SerializedPlutusScript;",
      ...
      "void [cardano, chain, allegra, alonzo, babbage, byron, conway, mary, multiEra, shelley, cip, cip25, cip36, cip4, cip8, cip67, cip129, plutus, uplc, core, crypto, datum, metadata, block, eraBlock, uplcResult, anySchema, cip4Checksum, cip8Envelope, packedCostModels];",
      "",
    ].join("\n"),
  );
