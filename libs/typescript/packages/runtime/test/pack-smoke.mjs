...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { applyParamsToScript, CIP4 as AggregateCIP4, CIP8Message as AggregateCIP8Message, Constr as AggregateConstr, Data as AggregateData, evaluatePhaseTwoRaw } from "@xray-network/xray-cardano-lib";',
      ...
      "    cardano.discover_required_witnesses !== chain.discover_required_witnesses ||",
      "    cardano.evaluatePhaseTwo !== plutus.evaluatePhaseTwo ||",
      "    cardano.SerializedPlutusScript !== plutus.SerializedPlutusScript ||",
      "    AggregateData !== Data || AggregateConstr !== plutus.Constr ||",
      "    AggregateCIP4 !== FocusedCIP4 ||",
      "    AggregateCIP8Message !== FocusedCIP8Message) {",
      '  throw new Error("packages did not preserve nominal class identity");',
      "}",
      'if ("apply_params_to_script" in cardano || "eval_phase_two_raw" in cardano ||',
      '    "apply_params_to_script" in plutus || "eval_phase_two_raw" in plutus) {',
      '  throw new Error("deprecated phase-two aliases remain exported");',
      "}",
      'if ("ProvisionalGovernanceActionId" in cardano || "ProvisionalGovernanceActionId" in cip) {',
      '  throw new Error("provisional CIP-129 binding leaked into a stable root");',
      "}",
      'if (!ProvisionalGovernanceActionId.from_parts(cardano.TransactionHash.from_raw_bytes(new Uint8Array(32)), 17).to_bech32().startsWith("gov_action1")) {',
      ...
      "}",
      'const cip4 = AggregateCIP4.calculateChecksum("13559f50b21a7229275ea7586e5b5a504c8ed1d9ddf17464c413f0ba");',
      ...
      'const cip8Envelope = AggregateCIP8Message.signData(cip8Address, cip8Payload, key.to_bech32());',
      'if (!AggregateCIP8Message.verifyData(cip8Address, key.to_public().hash().to_hex(), cip8Payload, cip8Envelope)) {',
      ...
      'if (cardano.CostModels !== chain.CostModels || JSON.stringify(JSON.parse(packedCostModels.to_json())) !== \'{"0":[1,-2,3],"1":[4],"2":[5]}\') {',
      ...
      "}",
      'if (AggregateData.to(undefined, AggregateData.Void()) !== "d87980" ||',
      '    AggregateData.from("d87980", AggregateData.Void()) !== undefined ||',
      '    !(new AggregateConstr(0, []) instanceof plutus.Constr)) {',
      ...
      "}",
      "if (cip.cip4.CIP4 !== FocusedCIP4 || cip.cip25.CIP25Metadata === undefined || cip.cip8.COSESign1Builder !== COSESign1Builder || plutus.Data !== Data) {",
  ...
  await writeFile(
    ...
    [
      'import * as cardano from "@xray-network/xray-cardano-lib";',
      'import { applyParamsToScript, Constr as AggregateConstr, Data as AggregateData, evaluatePhaseTwoRaw } from "@xray-network/xray-cardano-lib";',
      'import type { CIP4Checksum, CIP8MessageEnvelope, DataSchema, VoidSchema } from "@xray-network/xray-cardano-lib";',
      'import * as chain from "@xray-network/xray-cardano-lib-chain";',
      ...
      "const datumSchema = Data.Object({ owner: Data.Bytes(), amount: Data.Integer() });",
      "const voidSchema: VoidSchema = AggregateData.Void();",
      ...
      "type VoidValue = AggregateData.Static<typeof voidSchema>;",
      ...
      'const packedCostModels: chain.CostModels = cardano.CostModels.from_json(\'{"0":[1,-2,3]}\');',
      "const eraBlock = null as unknown as EraBlockJSON;",
      "Data.to(datumValue, datumSchema);",
      "AggregateData.to(voidValue, voidSchema);",
      "void new AggregateConstr(0, []);",
      "void applyParamsToScript;",
      "void evaluatePhaseTwoRaw;",
      "void cardano.evaluatePhaseTwo;",
      ...
      "void [cardano, chain, allegra, alonzo, babbage, byron, conway, mary, multiEra, shelley, cip, cip25, cip36, cip4, cip8, cip129, plutus, uplc, core, crypto, datum, metadata, block, eraBlock, uplcResult, anySchema, cip4Checksum, cip8Envelope, packedCostModels];",
      "",
    ].join("\n"),
  );
